                ...
                <div data-bs-spy="scroll" data-bs-target="#navbar-example3" data-bs-smooth-scroll="true"
                    ...
                    <div class="container row d-flex justify-content-center">
                        <form id="formForEndPayment" action="/AD_Network/MainController" method="get">
                            <input type="hidden" name="service" value="${requestScope.service.serviceId}">
                            <div class="container row d-flex justify-content-center">
                                <div class="col-7 sub-container d-flex flex-column justify-content-between" style="padding: 5px 0px;">
                                    <div>
                                        <table class="table table-striped table-hover text-center">
                                            ...
                                            <tbody>
                                                <c:choose>
                                                    <c:when test="${requestScope.typeBuy eq 'buyNow'}">
                                                        <c:set var="totalAmount" value="1"/>
                                                        <c:set var="totalPrice" value="${service.price * requestScope.transCycleBuyNow}"/>
                                                        <c:set var="transCycle" value="${requestScope.transCycleBuyNow}"/>
                                                        <tr>
                                                            <td>1</td>
                                                            <td>${requestScope.service.serviceName}</td>
                                                            <c:set var="pr" value="${requestScope.service.price}" />
                                                    ...
                                                    <c:otherwise>
                                                        <c:forEach var="cartInfor" items="${sessionScope.cart}">
                                                            ...
                                                            <c:set var="totalAmount" value="${totalAmount + cartInfor.value}"/>
                                                            <c:set var="transCycle" value="${requestScope.transCycle}"/>
                                                            <c:set var="pricePerService" value="${pricePerService + serviceInfor.getServiceForCartByName(cartInfor.key).price * cartInfor.value}"/>
                                                            <tr>
                                                                <td>${i}</td>
                                                                <td>${cartInfor.key}</td>
                                                                ...
                                                                <fmt:formatNumber var="price" value="${pr}" type="number" pattern="###,###"/>
                                                                <th>${price}đ/tháng</th>
                                                                <td>${cartInfor.value}</td>
                                                            </tr>
                                                        </c:forEach>
                                                        <c:set var="totalPrice" value="${totalPrice + pricePerService * requestScope.transCycle}"/>
                                                    </c:otherwise>
                                                </c:choose>
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="row mb3 text-center" style="padding: 12px; font-weight: bold">
                                        <div class="col">
                                            ...
                                            <label for="totalPrice" class="form-label align-items-center">Total Price: ${priceTotal}đ</label>
                                        </div>
                                        <div class="col">
                                            <label for="transCycle" class="form-label align-items-center">Cycle: ${transCycle}</label>
                                        </div>
                                        <div class="col">
                            ...
                            <!--value for action-->
                            <input type="hidden" name="action" value="endPayment">
                            <input type="hidden" name="totalPrice" value="${totalPrice}">
                            <input type="hidden" name="transCycle" value="${transCycle}">
                            <input type="hidden" name="totalAmount" value="${totalAmount}">
                            <input type="hidden" name="typeBuy" value="${requestScope.typeBuy}">
